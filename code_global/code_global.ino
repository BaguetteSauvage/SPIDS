...
////// the pump control //////////
int auto_pump = true; // by default, the pump is controlled automatically. We can also set a mode in which the pump is not used automatically and water will be pumped only if button pressed 
long long int delta_pump = 10 * 60000; // how long should we wait before watering the plant once more (in ms)
long long int  last_pump = - delta_pump; //the last time plants were watered. The initial value being 0, the hygrometry value will hhave timle to stabilise 
int time_pump = 500; //how long should we pump each time (ms)
